...
```
However, using likelihood ratio test (anova) --> There is statistically significant interaction between age and bmi, P value =  0.04983
...
Hence our preliminary final model is mv.cox.age.bmi.ia.

```{r}
prelim.final <- mv.cox.age.bmi.ia
```
...
mean(Data2$bmi)
```
###### Age vs bmi interaction (numerical vs numerical)

```{r}
coef_values <- coef(mv.cox.age.bmi.ia)
coef_age_bmi <- coef_values["age:bmi"]
mean_age_bmi <- exp(coef_age_bmi)
mean_age_bmi
```
